...
class GenerateModel extends Command
{
    ...
    {
        return [
            ['table',null,InputArgument::OPTIONAL],
        ];
    }
    ...
    public function fire()
    {
        ...
        $model_namespace      = 'App';
        $model_base           = 'Model';
        $model_base_namespace = 'Illuminate\\Database\\Eloquent\\Model';
...
";
                ...
                $relationships = "";
                $db_name       = DB::select('SELECT DATABASE()');
...
where TABLE_NAME = '{$table}'";
                ...
                foreach ($columns as $column) {
                    if ($column->Field != 'created_at' && $column->Field != 'updated_at'
                        && $column->Field != 'id' && $column->Field != 'deleted_at') {
                        $validators = [];
                        ...
                        if ($column->Null == 'NO') {
                            if (strpos($column->Type, 'varchar(') === false && strpos($column->Type,
                                    'text') === false) {
                                $validators[] = 'required';
                            }
                        }
                        ...
                        if (strpos($column->Type, 'varchar(') !== false) {
                            $length       = str_replace(')', '',
                                str_replace('varchar(', '', $column->Type));
                        ...
                        if (strpos($column->Type, 'decimal(') !== false || strpos($column->Type,
                                'double(') !== false || strpos($column->Type,
                                'float(') !== false) {
                            $validators[] = 'numeric';
                        }
